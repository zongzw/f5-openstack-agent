...
class LBaaSBuilder(object):
    ...
    sync_status = set([constants_v2.F5_PENDING_CREATE,
                       constants_v2.F5_PENDING_UPDATE,
                       constants_v2.F5_ERROR,
                       constants_v2.F5_ACTIVE])

    normal_status = set([constants_v2.F5_PENDING_CREATE,
                         constants_v2.F5_PENDING_UPDATE])
    ...
    def _assure_members(self, service, all_subnet_hints):
        ...
        for member in members:
            ...
            # pzhang(NOTE): when users delete a whole pool, the
            # members are all in ACTIVE status.
            if member_status in self.const_status or \
                    member_status == constants_v2.F5_ACTIVE:
